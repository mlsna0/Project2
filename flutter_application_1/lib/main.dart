...
import 'package:flutter_application_1/area_unit.dart';
import 'package:flutter_application_1/block.dart';
import 'package:flutter_application_1/block/block_l.dart';
import 'package:flutter_application_1/block/block_s.dart';
import 'package:flutter_application_1/block_provider.dart';
import 'package:flutter_application_1/coordinate.dart';
import 'package:flutter_application_1/game_theme.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  late List<List<AreaUnit>> gameArea;
  late List<List<AreaUnit>> gameAreaTemp;
  late List<List<AreaUnit>> gameAreaNextBlock;
  ...
  void initGameAreaTemp() {
    gameAreaTemp = <List<AreaUnit>>[];
    for (int i = 0; i < COUNT_ROW; i++) {
      List<AreaUnit> listAreaTemp = <AreaUnit>[];
  ...
  void initGameAreaNextBlock() {
    gameAreaNextBlock = <List<AreaUnit>>[];
    for (int i = 0; i < 4; i++) {
      List<AreaUnit> listAreaTemp = <AreaUnit>[];
      for (int j = 0; j < 4; j++) {
        listAreaTemp.add(AreaUnit());
  ...
  void initGameArea() {
    gameArea = <List<AreaUnit>>[];
    for (int i = 0; i < COUNT_ROW; i++) {
      List<AreaUnit> listArea = <AreaUnit>[];
      for (int j = 0; j < COUNT_COL; j++) {
...
//    gameArea[COUNT_ROW - 6][6] = AreaUnit(available: false, color: Colors.red);
  ...
  List<Widget> nextBlockContainer() {
    List<Widget> listRow = <Widget>[];
    for (int row = 0; row < 4; row++) {
      List<Widget> listCol = <Widget>[];
...
RaisedButton({required EdgeInsets padding, Color? color, required Text child, required Null Function() onPressed}) {
}
